...
#ifdef READER_NAGRA_MERLIN
		if (strlen((char*)mod1)) {
			mod1[sizeof(mod1) - 1] = strlen((char*)mod1);			memcpy(reader->mod1, mod1, sizeof(mod1));
			mod2[sizeof(mod2) - 1] = strlen((char*)mod2);			memcpy(reader->mod2, mod2, sizeof(mod2));
			key3588[sizeof(key3588) - 1] = strlen((char*)key3588);	memcpy(reader->key3588, key3588, sizeof(key3588));
			data50[sizeof(data50) - 1] = strlen((char*)data50);		memcpy(reader->data50, data50, sizeof(data50));
			mod50[sizeof(mod50) - 1] = strlen((char*)mod50);		memcpy(reader->mod50, mod50, sizeof(mod50));
			nuid[sizeof(nuid) - 1] = strlen((char*)nuid);			memcpy(reader->nuid, nuid, sizeof(nuid));
			cwpk[sizeof(cwpk) - 1] = strlen((char*)cwpk);			memcpy(reader->cwekey, cwpk, sizeof(cwpk));
		} else {
...
#ifdef READER_NAGRA_MERLIN
		if (strlen((char*)maxtv_mod1)) {
			maxtv_mod1[sizeof(maxtv_mod1) - 1] = strlen((char*)maxtv_mod1);				memcpy(reader->mod1, maxtv_mod1, sizeof(maxtv_mod1));
			maxtv_mod2[sizeof(maxtv_mod2) - 1] = strlen((char*)maxtv_mod2);				memcpy(reader->mod2, maxtv_mod2, sizeof(maxtv_mod2));
			maxtv_key3588[sizeof(maxtv_key3588) - 1] = strlen((char*)maxtv_key3588);	memcpy(reader->key3588, maxtv_key3588, sizeof(maxtv_key3588));
			maxtv_data50[sizeof(maxtv_data50) - 1] = strlen((char*)maxtv_data50);		memcpy(reader->data50, maxtv_data50, sizeof(maxtv_data50));
			maxtv_mod50[sizeof(maxtv_mod50) - 1] = strlen((char*)maxtv_mod50);			memcpy(reader->mod50, maxtv_mod50, sizeof(maxtv_mod50));
			maxtv_nuid[sizeof(maxtv_nuid) - 1] = strlen((char*)maxtv_nuid);				memcpy(reader->nuid, maxtv_nuid, sizeof(maxtv_nuid));
			maxtv_cwpk[sizeof(maxtv_cwpk) - 1] = strlen((char*)maxtv_cwpk);				memcpy(reader->cwekey, maxtv_cwpk, sizeof(maxtv_cwpk));
		} else {
			rdr_log(reader, "no keys built in, use config values mod1 + mod2 + key3588 + data50 + mod50 + nuid + cwekey");
		}
		reader->saveemm = (0 | reader->saveemm);
			...
			if (strncmp(current.atr, buf, 62) == 0) {
				...
				if (strlen((char*)v14_k1_generic)) {
					...
					if (!strlen((char*)reader->ins7E) && strlen((char*)v14_ins7E)) {
						v14_ins7E[sizeof(v14_ins7E) - 1] = strlen((char*)v14_ins7E);
						memcpy(reader->ins7E, v14_ins7E, sizeof(v14_ins7E));
					}
					if (!strlen((char*)reader->k1_generic)) {
						v14_k1_generic[sizeof(v14_k1_generic) - 1] = strlen((char*)v14_k1_generic);
						memcpy(reader->k1_generic, v14_k1_generic, sizeof(v14_k1_generic));
					}
					if (!strlen((char*)reader->k1_unique) && strlen((char*)v14_k1_unique)) {
						v14_k1_unique[sizeof(v14_k1_unique) - 1] = strlen((char*)v14_k1_unique);
			...
			if (strncmp(current.atr, buf, 62) == 0) {
				...
				if (strlen((char*)v15_k1_generic)) {
					...
					if (!strlen((char*)reader->ins7E) && strlen((char*)v15_ins7E)) {
						v15_ins7E[sizeof(v15_ins7E) - 1] = strlen((char*)v15_ins7E);
						memcpy(reader->ins7E, v15_ins7E, sizeof(v15_ins7E));
					}
					if (!strlen((char*)reader->k1_generic)) {
						v15_k1_generic[sizeof(v15_k1_generic) - 1] = strlen((char*)v15_k1_generic);
						memcpy(reader->k1_generic, v15_k1_generic, sizeof(v15_k1_generic));
					}
					if (!strlen((char*)reader->k1_unique) && strlen((char*)v15_k1_unique)) {
						v15_k1_unique[sizeof(v15_k1_unique) - 1] = strlen((char*)v15_k1_unique);
