...
rlmegae =       { "Redlight Mega Elite", 0, 12, "3F 77 18 00 00 C2 EB 41 02 6C 90 00",                                        35, 0, "\x01\x4D\x90\x2F", "\x00", "\x22\xD6\x0F\x37\x56\x7B\xAB\x12\xF5\x05\xD8\xC8\xC7\x1A\x22\xE3", "" },
rlmegar =       { "Redlight Mega Royale", 0, 12, "3F 77 18 00 00 D3 8A 42 01 64 90 00",                                       35, 0, "\x01\x4D\x90\x2F", "\x00", "\x79\x9E\xE9\x4F\xE7\xBC\x22\x11\x86\xCE\xD2\xB0\x6C\x83\x90\xB6", "" },
...
void findatr(struct s_reader *reader) {
	...
	if ( strncmp(current.atr, hdplus01.atr, hdplus01.atrsize) == 0 ) {
		...
		current.found = 1;
		return;
	} else if ( strncmp(current.atr, rlmegae.atr, rlmegae.atrsize) == 0 ) {
		memcpy(current.providername, rlmegae.providername, strlen(rlmegae.providername));
		memcpy(reader->boxkey, rlmegae.boxkey, 4);
		memcpy(reader->des_key, rlmegae.deskey, 16);
		memcpy(reader->pincode, "0000\0", 5);
		reader->saveemm = rlmegae.saveemm;
		reader->blockemm = rlmegae.blockemm;
		...
		memcpy(reader->boxkey, rlmegar.boxkey, 4);
		memcpy(reader->des_key, rlmegar.deskey, 16);
		memcpy(reader->pincode, "0000\0", 5);
		reader->saveemm = rlmegar.saveemm;
		reader->blockemm = rlmegar.blockemm;
		reader->boxkey_length = 4;
		reader->des_key_length = 16;
		reader->caid = 0x0500;
