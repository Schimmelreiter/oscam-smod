...
redlight =	{ "Redlight Elite", 0, 0, "3F 77 18 00 00 C2 EB 41 02 6C 90 00",                                              35, 0, "\x01\x4D\x90\x2F\x00", "\x00", "\x22\xD6\x0F\x37\x56\x7B\xAB\x12\xF5\x05\xD8\xC8\xC7\x1A\x22\xE3\x00", "" },
...
void findatr(struct s_reader *reader) {
	...
	if ( strncmp(current.atr, hdplus01.atr, hdplus01.atrsize) == 0 ) {
		...
		memcpy(reader->deskey, redlight.deskey, 33);
...
//		reader->des_key_length = 16;
		...
		memcpy(reader->deskey, tntviav5.deskey, 33);
		memcpy(reader->pincode, "0000\0", 5);
		...
		memcpy(reader->deskey, tntviav6.deskey, 33);
		memcpy(reader->pincode, "0000\0", 5);
